...
```
```{r}
USA_CC = components(bus_graph, mode="strong")
USA_CC$no
#USA_CC$csize
BigComp = which.max(USA_CC$csize)
BigComp

Main_USairports = induced_subgraph(bus_graph, 
    which(bus_graph$membership == BigComp))
Main_USairports
```
...
#log 1 = 0 
map_table$combined.f = log(map_table$between.f+1) + log(map_table$closeness.f+1) + log(map_table$eigen.f+1) + log(map_table$degree.f+1)
